...
import type { UploadFile, UploadConfig, UploadAdapter, UploadOptions } from '../lib/types';
import { extractFileMetadata, validateFile, generateFileId, resizeImage } from '../lib/utils';
...
export const useUpload = (options: UseUploadOptions = {}) => {
      ...
      try {
        ...
        if (config.resizeImages && file.type.startsWith('image/')) {
          ...
          if (maxWidth && maxHeight) {
            processedFile = await resizeImage(file, maxWidth, maxHeight, compressionQuality);
          }
        }
  ...
  const uploadSingleFile = useCallback(async (uploadFile: UploadFile) => {
    ...
    try {
      ...
      const progressInterval = setInterval(() => {
        setFiles(prev => {
          ...
          if (file && file.status === 'uploading' && file.progress < 90) {
            const newProgress = Math.min(file.progress + Math.random() * 20, 90);
      ...
      const result = await options.adapter.upload(uploadFile.file, uploadOptions);
