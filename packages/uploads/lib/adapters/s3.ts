...
import type { UploadAdapter, UploadOptions, UploadResult, S3Config } from '../types';
...
export class S3Adapter implements UploadAdapter {
  ...
  constructor(config: S3Config) {
    ...
    this.baseUrl = config.endpoint || `https://${config.bucket}.s3.${config.region}.amazonaws.com`;
  }
  ...
  async generatePresignedUrl(fileKey: string, expiresIn = 3600): Promise<string> {
