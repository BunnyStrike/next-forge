...
import type { UploadAdapter, UploadOptions, UploadResult, UploadThingConfig } from '../types';
...
export class UploadThingAdapter implements UploadAdapter {
  ...
  async upload(file: File, options?: UploadOptions): Promise<UploadResult> {
    try {
      ...
      if (options?.generateThumbnail && metadata.fileType === 'image') {
        try {
          ...
          if (thumbnailResponse.ok) {
            ...
            const thumbnailFile = Array.isArray(thumbnailResult) ? thumbnailResult[0] : thumbnailResult;
      ...
      throw new Error(`UploadThing upload failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async delete(fileUrl: string): Promise<void> {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`UploadThing delete error: ${response.status} - ${errorText}`);
      }
      ...
      throw new Error(`UploadThing delete failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
